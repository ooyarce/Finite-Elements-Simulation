...
infill = 0.98
densidad_PLA = 1.240*infill #gr/m3

import numpy as np

g = 9.8

fid = open("placa_prueba.msh","r")
...
for e in Quadrangles:
    ...
    properties_0["xi"] = -1.0
    properties_0["eta"] = -1.0
    sigma_xx[i,0] = sigma[0] 
    sigma_xy[i,0] = sigma[1]
    sigma_xx[i,0] = sigma[2]
    
    properties_0["xi"] = 1.0
    properties_0["eta"] = -1.0
    sigma_xx[i,1] = sigma[0] 
    sigma_xy[i,1] = sigma[1]
    sigma_xx[i,1] = sigma[2]

    properties_0["xi"] = 1.0
    properties_0["eta"] = 1.0
    sigma_xx[i,2] = sigma[0]
    sigma_xy[i,2] = sigma[1]
    sigma_xx[i,2] = sigma[2]
    
    properties_0["xi"] = -1.0
    properties_0["eta"] = 1.0
    sigma_xx[i,3] = sigma[0]
    sigma_xy[i,3] = sigma[1]
    sigma_xx[i,3] = sigma[2]

    properties_0["xi"] = 0.0
    properties_0["eta"] = -1.0
    sigma_xx[i,4] = sigma[0]
    sigma_xy[i,4] = sigma[1]
    sigma_xx[i,4] = sigma[2]

    properties_0["xi"] = 1.0
    properties_0["eta"] = 0.0
    sigma_xx[i,5] = sigma[0]
    sigma_xy[i,5] = sigma[1]
    sigma_xx[i,5] = sigma[2]   
    
    properties_0["xi"] = 0.0
    properties_0["eta"] = 1.0
    sigma_xx[i,6] = sigma[0]
    sigma_xy[i,6] = sigma[1]
    sigma_xx[i,6] = sigma[2]
    
    properties_0["xi"] = -1.0
    properties_0["eta"] = 0.0
    sigma_xx[i,7] = sigma[0]
    sigma_xy[i,7] = sigma[1]
    sigma_xx[i,7] = sigma[2]

    properties_0["xi"] = 0.0
    properties_0["eta"] = 0.0
    sigma_xx[i,8] = sigma[0]
    sigma_xy[i,8] = sigma[1]
    sigma_xx[i,8] = sigma[2]
...
write_element_node_data("sigma_x.msh",elementos,sigma_xx,"Sigma_x")
write_element_data("espesores.msh",elementos, espesores,"Espesores")
print (peso)
